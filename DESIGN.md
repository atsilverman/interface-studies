...
Play / pause / reset sit top-right of the stage (`useStageDock` + `StageControls`). On a phone they sit as a left floating cluster (same zinc-100/blur ring as the edit pencil). The desktop ↔ mobile toggle sits bottom-right of the stage on wide screens only. On a phone there is no layout toggle — always the mobile composition. **+** lives in the library rail (desktop) or the hamburger library panel (phone). On a phone a matching pencil disc sits bottom-right of the stage and opens Edit for the current study. They are not part of the study object. In-object controls use `IconButton` only when they belong to the metaphor. No speed chips.
...
- Runtime overlay cycles filing statuses (`FILE_STEPS` / `EDIT_STEPS` / `REMIX_STEPS` in `catalog.ts`) with a progress bar. On desktop it floats at the bottom of the stage. On a phone it sits **under the Interface Studies / hamburger row**, not over the study. When a Cursor build is ready it stamps **Live** and the page reloads so the new study is unmistakable. While it is still building, **Cancel** on the overlay stops the agent and reverts the brief.
