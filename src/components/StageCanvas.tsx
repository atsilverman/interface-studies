...
export function StageCanvas({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const width = Math.max(content.scrollWidth, content.offsetWidth, frameWidth);
      const height = Math.max(content.scrollHeight, content.offsetHeight);
      const next = Math.min((hostBox.width - 16) / width, (hostBox.height - 16) / height, 1);
      setScale(Number.isFinite(next) && next > 0 ? Math.max(0.38, next) : 1);
    };
